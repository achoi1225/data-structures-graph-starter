...
function friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance) {
  ...
  visited.add(name);
  currentDistance += 1;
  adjacencyList[name].forEach(friend => friendsOfRecursion(friend, adjacencyList, visited, maxDistance, currentDistance));
}
...
function friendsOf(adjacencyList, name, distance) {
  ...
  while(queue.length) {
    friendsOfRecursion(queue.shift(), adjacencyList, visited, distance, 0);
    // currentDistance++;
  }
  // original person's name is still in this array.
  visited.delete(name);
